...
public abstract class MARSAbstract {

    protected static int[] K;
    protected static final int KEY_COUNT = 40;
    protected static final int HEX_255 = 0xff;
    ...
    MARSAbstract(byte[] key){
        K = expandKey(key);
    }


    protected int rotateLeft(int val, int pas) {
        int a = val << pas;
        int b = val >>> (32 - pas);
        return a | b;
    }

    protected int rotateRight(int val, int pas) {
        int a = val >>> pas;
        int b = val << (32 - pas);
        return a | b;
    }
    ...
    protected int[] expandKey(byte[] key) {
        int[] tmp = new int[KEY_COUNT];
        int[] t = new int[15];
        int[] B = {0xa4a8d57b, 0x5b5d193b, 0xc8a8309b, 0x73f9a978};

        int n = key.length >> 2;
        for (int i = 0; i < key.length; i++)
            t[i / 4] |= (key[i] & HEX_255) << (i * 8);
        t[n] = n;

        for (int j = 0; j < 4; j++) {
            for (int i = 0; i < 15; i++)
                t[i] ^= rotateLeft(t[(i + 8) % 15] ^ t[(i + 13) % 15], 3) ^ (4 * i + j);

            for (int ii = 0; ii < 4; ii++)
                for (int i = 0; i < 15; i++)
                    t[i] = rotateLeft(t[i] + s_box[t[(i + 14) % 15] & 0x1FF], 9);
    ...
    public abstract byte[] encryptBlock(byte[] in);
